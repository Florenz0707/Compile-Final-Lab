...
% 修复xeCJK的verbatim字体缩放警告
% 设置CJK等宽字体的缩放比例，解决xeCJKVerbAddon警告
% 根据警告信息，设置Scale=0.9896到CJKfamily \CJKttdefault
\setCJKmonofont{FangSong}[Scale=0.9896]

% 修复FangSong字体斜体形状未定义的警告
% 使用xeCJK包提供的字体设置，将斜体形状映射到正常形状
% 这样可以避免"Font shape `TU/FangSong(0)/m/it' undefined"警告
% 设置FangSong字体的斜体形状为正常形状（因为FangSong没有真正的斜体）
...
\begin{enumerate}
    \item \textbf{初始化阶段}：
    \begin{itemize}
        \item 初始化关键字表（int, void, return, const, float, if, else）
...
\subsubsection{符号表和作用域管理} % 对应源码文档 3.3.4
...
\subsubsection{短路求值实现} % 对应源码文档 3.3.9
...
\subsubsection{IR生成完整流程} % 对应源码文档 3.3.11
...
\subsubsection{IR生成示例} % 对应源码文档 3.3.12
...
\subsubsection{代码实现细节} % 对应源码文档 3.3.13
...
\subsubsection{设计思想总结} % 对应源码文档 3.3.14
